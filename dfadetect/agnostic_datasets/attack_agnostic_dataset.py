...
class AttackAgnosticDataset(SimpleAudioFakeDataset):
    ...
    def oversample_dataset(self):
        ...
        if diff_length > 0:
            # Already correct — uses tuple syntax
    ...
    def undersample_dataset(self):
        ...
        if spoof_length > bona_length:
            # FIX: use ("spoof",) and ("bonafide",) — length-1 tuples required
            # by Pandas when groupby() was called with a list (by=['label'])
            # OLD (broken): samples.get_group("spoof").sample(bona_length, replace=True)
            # OLD (broken): samples.get_group("bonafide")
            spoofs = samples.get_group(("spoof",)).sample(bona_length, replace=True)
            self.samples = pd.concat(
                [samples.get_group(("bonafide",)), spoofs],
                ignore_index=True
            )

    def get_bonafide_only(self):
        samples = self.samples.groupby(by=['label'])
        # FIX: tuple syntax required
        # OLD: samples.get_group("bonafide")
